...
const JOYSTICK_X: libc::c_uint = 0x30;
const JOYSTICK_Y: libc::c_uint = 0x31;

// Default button bindings - as configured in our emulator. vJoy is one-based with buttons.
//@todo capture these in some enum data type and make that public
pub const BUTTON_A: libc::c_uchar = 0x01;
...
fn is_vjoystick_n64(index: libc::c_uint) -> bool {
    if !does_vjoystick_axis_exist(index, JOYSTICK_X) { return false; }
    if !does_vjoystick_axis_exist(index, JOYSTICK_Y) { return false; }
...
fn populate_vn64c(controller: &mut VirtualN64Controller) -> bool {
    ...
    controller.props.x_min = get_vjoystick_axis_min(controller.props.vjoy_device_number, JOYSTICK_X);
    controller.props.x_max = get_vjoystick_axis_max(controller.props.vjoy_device_number, JOYSTICK_X);
    controller.props.y_min = get_vjoystick_axis_min(controller.props.vjoy_device_number, JOYSTICK_Y);
    controller.props.y_max = get_vjoystick_axis_max(controller.props.vjoy_device_number, JOYSTICK_Y);
...
fn flush(controller: &VirtualN64Controller) -> bool {
    if !set_vjoystick_axis(controller.props.vjoy_device_number, JOYSTICK_X, controller.state.x) {
        return false;
    }
    if !set_vjoystick_axis(controller.props.vjoy_device_number, JOYSTICK_Y, controller.state.y) {
        return false;
    }
...
// just modifying the vjoystick ourselves
pub fn set_button(controller: &VirtualN64Controller, button: libc::c_uchar, value: bool) -> bool { 
    set_vjoystick_button(controller.props.vjoy_device_number, button, value as libc::c_int);
    
    println!("Setting button {} to {}", button, value as libc::c_int);
